          ...
          <dl>
            ...
            <dd class="c-s-dl-li">
              <ul class="clearfix">
                ...
                <li v-for="(subject,index) in subjectNestedList" :key="index" :class="{active:oneIndex}">
...
export default {
  ...
  created(){
      //获取课程列表
      this.selectInitCourse();
      //获取分类
      this.getNestedTreeList2();
  },
  methods:{
      //查询课程列表
      selectInitCourse(){
        course.getPageList(1,8,searchObj)
                .then(response=>{
      ...
      gotoPage(page){
          this.page=page;
          course.getPageList(page,8,searchObj)
                .then(response=>{
                  this.data=response.data.data;
                })
      },
